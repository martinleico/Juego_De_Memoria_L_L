...
public class Utilities {
    ...
    //METODO DONDE SE CARGAN NUMEROS RANDOM EN FILA
    ...
    public static List<Integer> rowsTableAll(int nivelDificultad) {
        List<Integer> all = new ArrayList<>();
        all.addAll(rowsTable());
        all.addAll(rowsTable());

        // Determinar la cantidad de filas según el nivel de dificultad
        int cantidadFilas;
        if (nivelDificultad == 1) { // Fácil
            cantidadFilas = 4;
        } else if (nivelDificultad == 2) { // Medio
            cantidadFilas = 6;
        } else if (nivelDificultad == 3) { // Difícil
            cantidadFilas = 8;
        } else { // Nivel de dificultad inválido, usar valor por defecto
            cantidadFilas = 4;
        }
